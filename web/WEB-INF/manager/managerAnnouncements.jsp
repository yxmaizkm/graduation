...
<script>
    layui.use(['table','layer'], function(){
        ...
        table.render({
            elem: '#demo'
            ,height: 312
            ,url: '${pageContext.request.contextPath}/manager/announce/list' //数据接口
            ,page: true //开启分页
            ,cols: [[ //表头
                {field: 'id', title: 'ID', sort: true, fixed: 'left'}
                ,{field: 'title', title: '标题', sort: true}
                ,{field: 'createTime', title: '创建时间', templet: "<div>{{layui.util.toDateString(d.date, 'yyyy-MM-dd HH:mm:ss')}}</div>"}
        ...
        table.on("tool(test)",function (obj) {
            ...
            if(layEvent === 'detail'){
                $.ajax({
                    type : "POST",
                    url : "${pageContext.request.contextPath}/manager/announce/detail",
                    data : {
                        id:data.id,
                        title:data.title,
                ...
                $.ajax({
                    type : "POST",
                    url : "${pageContext.request.contextPath}/manager/announce/toUpdate",
                    data : {
                        id:data.id,
                        title:data.title,
                        message:data.message,
                ...
                layer.confirm('您确定要删除吗？', {
                    ...
                    $.ajax({
                        type : "POST",
                        url : "${pageContext.request.contextPath}/manager/announce/delete",
                        data : {
                            id:data.id,
                            },
                        dataType:'text',
